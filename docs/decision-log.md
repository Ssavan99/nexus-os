...
## [2026-06-25] Phase 2E: Unify ChatGPT onto the Always-On Native HTTP MCP

Decision: migrate ChatGPT off the Phase 2B path (`nexus-gbrain-readonly-mcp` wrapper bridged via `openai-tunnel-client`, which required per-session terminal commands) onto the same always-on `gbrain serve --http` endpoint that serves claude.ai. ChatGPT Developer Mode supports custom remote MCP connectors over OAuth, and individual tiers are read/fetch-only — matching our read-only posture.

Implementation: registered a separate `read`-scoped public PKCE client `chatgpt-readonly` (grants `authorization_code,refresh_token`, token-endpoint auth `none`) with ChatGPT's per-connector redirect URI `https://chatgpt.com/connector/oauth/<id>` (read off the New App dialog). In ChatGPT: Developer Mode → custom connector → URL `https://plaster-paving-water.ngrok-free.dev/mcp` → Authentication OAuth → Registration method **User-Defined OAuth Client** → paste Client ID, no secret. DCR stays off (and ChatGPT confirmed DCR/CIMD unavailable for our server, so manual client is the path). Verified 2026-06-25: read returns brain results from ChatGPT.

Outcome: one always-on endpoint now serves both Claude (chat + code) and ChatGPT; each cloud surface has its own independently-revocable read-scoped client. The wrapper + tunnel-client are retired for daily use (code kept as fallback in `/Users/ssavan99/MCPs`). New ChatGPT sessions need zero manual commands. Confirmed (2026-06-25) the launchd stack auto-starts at login: `postgresql@17` (brew service) + `com.nexus.claude-chat-gbrain` (RunAtLoad) + persisted ngrok authtoken.

## [2026-06-27] Phase 3 Foundation: Local Embeddings via Ollama

Decision: enable semantic search using a LOCAL embedding model (Ollama `nomic-embed-text`, 768-dim) rather than an API provider. Rationale: at this brain's scale local quality ≈ OpenAI, it's light on an M3/16GB (embedded 299 chunks in ~15s, near-zero load), it's $0, and brain content never leaves the machine (consistent with the read-only-cloud privacy posture). API (OpenAI text-embedding-3-small) would cost ~$0.003 one-time + incremental pennies but sends page text + queries out.

Mechanics (Postgres): the brain was init'd embeddings-disabled with a `content_chunks.embedding vector(1280)` column. Since 0 chunks were embedded (nothing to lose), resized the column 1280→768 via gbrain's `docs/embedding-migrations.md` Postgres recipe (drop HNSW index → ALTER COLUMN TYPE vector(768) → recreate HNSW), then set `embedding_model: ollama:nomic-embed-text` + `embedding_dimensions: 768` in `~/.gbrain/config.json` (the file-plane field; `gbrain config set embedding_model` is a guarded DB no-op). `gbrain embed --stale` → 299/299. Ollama runs as a `brew services` daemon (auto-start at login); needed at query time for semantic search (keyword search works without it). Restarted the always-on HTTP service to pick up the new config so cloud agents get semantic search too.

## [2026-06-27] Cross-Agent Plan Reference: Repo Canonical + Brain Mirror

Decision: the operating plan lives in two synced places because the repo and the brain reach different agents. Canonical: `docs/master-plan.md` (repo, version-controlled) — reachable by local agents (Codex, Claude Code). Mirror: vault page `wiki/areas/nexus-os/master-plan.md`, imported into GBrain — reachable by cloud agents (claude.ai, ChatGPT) which see only the brain. Any agent in a fresh session/chat can be pointed at the plan: local agents read the repo doc; cloud agents `search "nexus master plan"`. When the plan changes, update the repo doc and re-`gbrain put` the vault mirror (repo wins on conflict). This is how work continues across context-window resets and tool switches (Claude ↔ ChatGPT ↔ Codex).

## [2026-07-01] Nexus v1: Vault→Brain Sync via Idempotent Import (no git)

Decision: keep the GBrain brain current with the Obsidian vault using a periodic `gbrain import` under launchd, NOT `gbrain sync` (which requires the vault to be a git repo). Rationale: the vault lives inside Google Drive; git-initializing it would make Drive sync the `.git` folder (corruption risk) and Drive already provides backup + file version history. Uploading the private vault to GitHub was rejected (public exposes private content; private is unnecessary cloud exposure). `gbrain import` is idempotent — upserts by slug, content-hash short-circuits unchanged pages — so re-running is cheap (~2.6s for 77 files, only changed notes re-embed/extract). Local Postgres makes concurrent import + always-on `serve --http` safe.
