...
- Phase 2C status: active. Claude Code is wired to raw local GBrain MCP via `~/.claude/settings.json` (global). Operating instructions at `docs/claude-code-gbrain-operating-instructions.md`. Smoke test pending first session with MCP available.
