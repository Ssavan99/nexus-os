...
- Phase 2D status: 2D.1 verified. claude.ai is connected to GBrain's **native** `serve --http` (OAuth 2.1) via a Cloudflare quick tunnel, using a manually pre-registered `read`-scoped public PKCE client (DCR off) — no custom proxy. End-to-end acceptance passed: read works, write refused (`insufficient_scope`). Helper: `scripts/claude-chat-gbrain-tunnel.sh`. Remaining: 2D.2 always-on (named tunnel + launchd; single-owner avoids the PGLite lock). Plan + runbook at `docs/claude-chat-gbrain-plan.md`.
